...
final class BPackageAPIClient {
    ...
    private func bPackageRequestEnvelope(bPackagePath: String,
        ...
        let bPackageMessage = (bPackageObject["message"] as? String) ?? ""
        let bPackagePayload = bPackageObject["result"] ?? bPackageObject["data"] ?? NSNull()
        bPackageLogDebugPayload(
            bPackageCategory: "响应Envelope",
            bPackageMessage: "code=\(bPackageCode)，message=\(bPackageMessage)，result/data=\(String(describing: bPackagePayload))"
        )
        return BPackageRawEnvelope(bPackageCode: bPackageCode,
                                   bPackageMessage: bPackageMessage.isEmpty ? nil : bPackageMessage,
    ...
    private func bPackageLogDebugPayload(bPackageCategory: String,
        ...
        guard bPackageIsRuntimeDebug else { return }
        BPackageLogger.bPackageShared.bPackageLog(bPackageCategory, bPackageMessage)
    }

    private var bPackageIsRuntimeDebug: Bool {
        bPackageConfiguration.bPackageDebugFlag == 1
    }
